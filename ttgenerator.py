from openpyxl import load_workbook
from json import dump
import pandas as pd
from json import load
from html2image import Html2Image
from PIL import Image
import numpy as np
import sys

def isin_list(to_check : str, check_list : list[list[str]]):
    for item in check_list:
        if item[0].strip().lower() in to_check.lower() and item[1].strip().lower() in to_check.lower():
            return True
    return False

def triplecheck_pass(to_check : str, check_list : list[list[str]]):
    for item in check_list:
        if len(item) != 3:
            continue
        elif item[0].strip().lower() in to_check.lower() and item[1].strip().lower() in to_check.lower() and item[2].strip().lower() in to_check.lower():
            return True

    return False

wb_name = "timetable.xlsx"

# print("Removing excess sheets from timetable.xlsx...") 

print("Opening timetable.xlsx...")

wb = load_workbook(wb_name)

# excess_sheets = wb.sheetnames[1:]

# for i in excess_sheets:
#     wb.remove(wb[i])

# ws = wb.worksheets[0]

# # col = 1
# # for i in range(10):
# #     cell = ws.cell(row=2, column=col)
# #     if (cell.value == "BDS-2020"):
# #         break
# #     col += 1

print("Opening the timetable sheet...")

ws = wb.worksheets[0]

courselist = []
degree = ""

print("Reading courses...")

with open("pref.txt", 'r') as f:
    degree = f.readline().strip("\n").strip()
    courselist = [line.split("-") for line in f.readlines()]

timings = ["8:30", "10:00", "11:30", "1:00", "2:30", "4:00", "5:30", "7:00"]

timings_dict = {time:None for time in timings}

days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]

ttable = {}
for day in days:
    ttable[day] = timings_dict.copy()

print("Parsing the timetable for given courses...")

timings_idx = 0
clash_flag = False
courses_added = len(courselist)

for col in range(6, 6+(9*8),9):

    for row in range(5, 245):
        curr_cell = ws.cell(row=row, column=col)
        if (curr_cell.value == None):
            continue
        if ((degree.lower() in curr_cell.value.lower() and isin_list(curr_cell.value, courselist)) or triplecheck_pass(curr_cell.value, courselist)):
            newrow = row
            
            while (str(ws.cell(row=newrow, column=1).value) == "None" and newrow > 0):
                newrow -= 1
            
            day = str(ws.cell(row=newrow, column=1).value)
            day = day.strip().split(" ")[0]
            if (ttable[day][timings[timings_idx]] != None):
                print(ttable)
                print(f"ERROR: {curr_cell.value} clashing with {ttable[day][timings[timings_idx]]}")
                clash_flag = True
                break

            ttable[day][timings[timings_idx]] = str(curr_cell.value) + f" [VENUE:{str(ws.cell(row=row, column=2).value)}]"
            if ("lab" in str(curr_cell.value).lower()):

                if (ttable[day][timings[timings_idx + 1]] != None):
                    print(f"ERROR: {curr_cell.value} clashing with {ttable[day][timings[timings_idx + 1]]}")
                    clash_flag = True
                    break

                ttable[day][timings[timings_idx + 1]] = str(curr_cell.value) + f" [VENUE:{str(ws.cell(row=row, column=2).value)}]"
            else:
                courses_added -= 1

        if clash_flag:
            break
        
    timings_idx += 1
        
if clash_flag:
    sys.exit(1)

if courses_added > 0:
    print("Not all courses were added, this may be because the courses written in pref.txt were not correct.")
    print("Kindly recheck pref.txt and/or report this bug. Generating timetable with remaining courses...")
else:
    print("Time table successfully read. Creating pictorial timetable...")

indices = list(ttable.keys())
timings = list(ttable[indices[0]].keys())
newtimings = [f"{timings[i]} - {timings[i + 1]}" for i in range(len(timings) - 1)]
newtimings.append(timings[-1])


time_dict = {time:[ttable[idx][time] if ttable[idx][time] != None else np.nan for idx in indices] for time in timings}


df = pd.DataFrame(time_dict)

df["Days"] = indices
df.set_index("Days", inplace=True)
df.index.name = None

last_nempty = 0
for i in range(len(df.columns)):
    if (not df.iloc[:, i].isnull().all()):
        last_nempty = i

print(last_nempty, len(df.columns))

df.rename(columns=dict(zip(timings, newtimings)), inplace=True)
df.drop(columns=df.columns[last_nempty + 1:], inplace=True)
df.fillna("", inplace=True)

df.to_html("index.html")

hti = Html2Image()

colors = open("colors.txt", 'r').readlines()
htext_color = colors[0].strip("\n").strip()
btext_color = colors[1].strip("\n").strip()
bg_color = colors[2].strip("\n").strip()
time_color = colors[3].strip("\n").strip()
day_color = colors[4].strip("\n").strip()


css = "table {background: %s;} th {color: %s; width: 200px;} td {color: %s; width: 200px;} thead {background: %s;} tbody > tr > th {background: %s;} tr {height: auto;}" % (
    bg_color, htext_color, btext_color, time_color, day_color)
html = open("index.html", 'r').read()
html = html.replace("text-align: right;", "text-align: center;")

hti.screenshot(html_str=html, css_str=css, save_as="ttable.png", size=(1080, 600))

img = Image.open("ttable.png")
bbox = img.getbbox()
img = img.crop(bbox)
img.save("ttable.png") 

###########################################################


print("COMPLETED")
